...
import {
  Bell, User, Copy, Check, RefreshCw, Terminal, ExternalLink,
  Download, Clock, CheckCircle, XCircle, Zap,
} from 'lucide-react';
...
export default function SellerApi() {
  ...
  const [activeEvents, setActiveEvents] = useState<Record<string, boolean>>(
    Object.fromEntries(events.map(e => [e.id, e.active]))
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="px-3 py-1.5 bg-[#A3E635] text-black text-xs font-semibold rounded-lg hover:bg-[#95d630] transition-colors">
                Generate New
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            ...
            <div className="space-y-4">
              ...
              <button className="w-full py-2.5 bg-[#A3E635] text-black text-sm font-semibold rounded-xl hover:bg-[#95d630] transition-colors">
                Update Endpoint
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#A3E635]">
                  <th className="text-left py-3 px-4 text-black font-semibold text-xs uppercase tracking-wider">Time</th>
                  <th className="text-left py-3 px-4 text-black font-semibold text-xs uppercase tracking-wider">Event</th>
                  <th className="text-left py-3 px-4 text-black font-semibold text-xs uppercase tracking-wider">Status</th>
                  <th className="text-left py-3 px-4 text-black font-semibold text-xs uppercase tracking-wider">Code</th>
                </tr>
              </thead>
              <tbody>
                {recentEvents.map((ev, i) => (
                  <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-4 text-slate-700">{ev.time}</td>
                    <td className="py-3 px-4 font-medium text-slate-900">{ev.event}</td>
                    ...
                    <td className="py-3 px-4 text-slate-700 font-mono">{ev.code}</td>
                  </tr>
                ))}
              </tbody>
            </table>
